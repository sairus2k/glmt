...
go test -v -tags e2e -count=1 -timeout 25m ./e2e/...
...
- **`log/`** — `FileLogger` writes JSONL session logs. `LoggingClient` wraps `Client` for transparent API call logging with token redaction.
- **`demo/`** — Mock client for demo mode.

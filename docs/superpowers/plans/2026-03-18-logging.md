...
func TestFileLogger_WritesJSONLines(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(lines[2]), &step2))
	assert.Contains(t, step2["msg"], "[REDACTED]")
	assert.NotContains(t, step2["msg"], "secret-tok")

	// Run end line
	var end map[string]any
	require.NoError(t, json.Unmarshal([]byte(lines[3]), &end))
...
This task wires the `--log` flag in `main.go` for both non-interactive and TUI modes, and adds the `FileLogger` field to `AppModel`. This is done in one task to avoid intermediate compile errors (changing `run()` call signatures requires both callers and callees to match).
...
Complete new `runNonInteractive`:
...
New `runTUI` (adds `enableLog` param, FileLogger setup, and `defer` cleanup):
...
func runTUI(flagHost, flagToken string, flagProjectID int, enableLog bool) error {
	...
	// Set up file logging if requested and credentials available
	var fileLogger *glmtlog.FileLogger
	if enableLog {
		if creds == nil {
			...
			if err != nil {
				...
				fileLogger = fl
				defer fileLogger.Close() // cleanup if user never starts a train
			}
		}
	}
...
git commit -m "feat: wire --log flag in non-interactive and TUI modes"
```
...
Replace the `startTrain` method in `internal/tui/app.go` with logging support. The `FileLogger` field and `glmtlog` import were added in Task 5.
...
func (m *AppModel) startTrain(mrs []*gitlab.MergeRequest) tea.Cmd {
	...
	runCmd := func() tea.Msg {
		...
		// Wrap client locally for API logging (don't mutate m.client)
		var runClient gitlab.Client = client
		if fileLogger != nil {
			runClient = glmtlog.NewLoggingClient(client, fileLogger)
		}
...
Note: `fileLogger.Close()` is NOT called here — it's deferred in `runTUI` (main.go) to handle the case where the user exits without starting a train. The `defer` in `runTUI` ensures cleanup in all exit paths.
...
git commit -m "feat: update startTrain with composite logger and API logging"
```
